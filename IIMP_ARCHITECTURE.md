...
CHANGED | GitHub Copilot (GPT-5.4) | REPOSITORY_PUBLICATION | Chuẩn hóa repository để publish lên GitHub:
- Khởi tạo Git repository cục bộ, nối với remote `vuquangvinh1004/IIM-Platform` và publish source lên `origin/main`
- Cập nhật `.gitignore` để loại `node_modules/`, cache runtime và dữ liệu bản đồ cỡ lớn khỏi lịch sử push
- Loại file `modules/logistics/pso_logistics_map/vietnam-260413.osm.pbf` khỏi lịch sử publish vì vượt giới hạn 100 MB của GitHub

ADDED | GitHub Copilot (GPT-5.4) | AUTOMATION_WORKFLOWS | Bổ sung GitHub Actions workflows ở `.github/workflows/`:
- `python-ci.yml`: chạy core tests, integration tests, shell smoke test và một lát cắt representative module tests trên Windows
- `design-lint.yml`: lint `DESIGN.md` bằng `@google/design.md` trên Node.js 22
- `windows-package-smoke.yml`: workflow thủ công để build PyInstaller package và upload artifact `dist/`
