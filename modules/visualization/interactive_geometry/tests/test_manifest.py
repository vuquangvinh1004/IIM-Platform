"""Manifest validation tests for the interactive_geometry module."""
...
def test_manifest_parses_successfully():
    ...
    assert manifest.id == "interactive_geometry"
    assert manifest.entry_point.endswith(":InteractiveGeometryModule")
...
def test_manifest_has_tags():
    raw = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    assert "geometry" in raw.get("tags", [])
    assert "3d" in raw.get("tags", [])
