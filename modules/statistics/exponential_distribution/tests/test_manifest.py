"""Tests for exponential_distribution module manifest validation."""
...
def test_manifest_parses_successfully():
    ...
    assert manifest.id == "exponential_distribution"
    assert manifest.entry_point.endswith(":ExponentialDistributionModule")
...
def test_manifest_category_statistics():
    raw = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    manifest = ModuleManifest(**raw)
    assert manifest.category == "statistics"


def test_manifest_version_parseable():
    raw = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    manifest = ModuleManifest(**raw)
...
def test_manifest_invalid_without_required_field():
    raw = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    del raw["entry_point"]
