"""Tests for qc_inspection manifest file."""
...
def test_manifest_id():
    data = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    assert data["id"] == "qc_inspection"


def test_manifest_entry_point():
    data = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    assert "QCInspectionModule" in data["entry_point"]
...
def test_manifest_export_settings():
    data = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    assert "supports_export" in data
